...
GOOGLE_CLIENT_ID=seu-client-id-google-opcional
```
...
- `GOOGLE_CLIENT_ID` habilita o botao "Continuar com Google"; sem ele o app informa que o provedor esta indisponivel no ambiente atual.

## Testes Automatizados

O projeto possui testes de autenticacao, CRUD principal, isolamento por usuario, protecao contra XSS no frontend, PWA e travas de configuracao de producao.

```bash
python -m pytest
```

Esses testes usam um banco SQLite temporario e nao alteram o banco local de desenvolvimento.
...
- Em `APP_ENV=production`, o app bloqueia startup com `SECRET_KEY` fraca, `ALLOWED_ORIGINS=*`, `PUBLIC_BASE_URL` local ou banco SQLite.
